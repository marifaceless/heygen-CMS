...
import { LicenseRecord, validateLicenseKey } from '../license';
...
export const LicenseGate: React.FC<LicenseGateProps> = ({ licenses, licenseLoadError, onUnlock }) => {
  const [key, setKey] = useState('');
  const [error, setError] = useState('');
  const canValidate = licenses.length > 0 && !licenseLoadError;

  const handleValidate = () => {
    const result = validateLicenseKey(key, licenses);
    if (result.ok) {
      localStorage.setItem('heygen_license_active', 'true');
      localStorage.setItem('heygen_license_key', result.license.key);
      onUnlock();
    } else {
        ...
        <div className="space-y-6">
          <div className="relative">
            ...
            <input
              ...
              className="w-full px-5 py-4 border border-slate-200 rounded-2xl focus:ring-4 focus:ring-blue-50 focus:border-blue-500 outline-none transition-all uppercase font-mono text-center tracking-[0.2em] placeholder:text-slate-300 disabled:bg-slate-50 disabled:text-slate-300"
            />
            {licenseLoadError && (
              <div className="mt-3 flex items-center gap-2 text-amber-600 bg-amber-50 p-3 rounded-xl border border-amber-100">
                <ICONS.Info className="w-4 h-4" />
                <p className="text-xs font-semibold">{licenseLoadError}</p>
              </div>
            )}
            {error && (
          ...
          <button
            ...
            disabled={!canValidate}
            className="w-full bg-blue-600 hover:bg-blue-700 text-white font-bold py-4 rounded-2xl shadow-xl shadow-blue-100 transition-all active:scale-[0.98] flex items-center justify-center gap-2 disabled:bg-slate-300 disabled:shadow-none disabled:cursor-not-allowed"
          >
